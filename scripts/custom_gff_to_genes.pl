...

{
    ...
    print join("\t",qw{locusId sysName type scaffoldId begin end strand gene_name desc})."\n"; ####added gene_name
    my %sysNameSeen = ();
    ...
    ####added this code block to extact gene names
    my $gene_name = "";
    if ($attributes =~ m/gene=(.*?);/) {
        $gene_name = $1;
    } else {
        $gene_name = "";
    }
    ...
    print join("\t", $locusId, $sysName, $out_type, $scaffold, $begin, $end, $strand, $gene_name, $desc)."\n"; ####added gene name
    }
}
